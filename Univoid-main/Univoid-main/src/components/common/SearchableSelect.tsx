...
export function SearchableSelect({
  ...
  const handleManualSubmit = useCallback(() => {
    if (manualValue.trim()) {
      onSelect({ id: `manual_${Date.now()}`, name: manualValue.trim() });
      setIsOpen(false);
      setShowManualInput(false);
      setManualValue("");
      setSearchTerm("");
  ...
  const handleItemTap = useCallback((item: { id: string; name: string }) => (e: React.MouseEvent | React.TouchEvent) => {
    e.preventDefault();
    e.stopPropagation();
    handleSelect(item);
  }, [handleSelect]);

  // Render dropdown content (shared between desktop and mobile sheet)
        ...
        <div className="max-h-[50vh] overflow-y-auto overscroll-contain p-2 -webkit-overflow-scrolling-touch">
          ...
          {results.length > 0 ? (
            <>
              {results.map((item) => (
                <button
                  ...
                  className={cn(
                    "w-full flex items-center justify-between gap-2 px-3 py-3.5 rounded-xl text-left transition-all duration-150",
              ...
              <div className="border-t border-border mt-2 pt-2">
                ...
                >
                  ...
                  <span className="font-medium">Not in list? Type manually</span>
                </button>
              </div>
            </>
          ) : hasSearched ? (
            <div className="py-4 text-center space-y-3">
              ...
              <div className="space-y-2">
                <button
                  ...
                  onTouchEnd={(e) => {
                    e.preventDefault();
                    handleSelect({ id: `custom_${searchTerm}`, name: searchTerm });
                  }}
                  className={cn(
            ...
            <div className="py-6 text-center space-y-3">
              ...
              {/* Manual Entry Option when no search */}
              <button
                type="button"
                onClick={handleManualEntry}
                onTouchEnd={(e) => {
