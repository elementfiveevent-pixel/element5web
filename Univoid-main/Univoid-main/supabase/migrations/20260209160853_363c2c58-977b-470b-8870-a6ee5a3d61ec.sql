...
-- Fix RPC to block rejected users from re-registering
...
BEGIN
  ...
  IF v_existing_registration IS NOT NULL THEN
    ...
    IF v_existing_registration.payment_status = 'rejected' THEN
      RETURN jsonb_build_object(
        'success', false,
        'error', 'REGISTRATION_REJECTED',
        'message', 'Your registration for this event was rejected. You cannot re-apply.'
      );
    END IF;
  ...
  INSERT INTO event_registrations (
    event_id, user_id, custom_data, payment_screenshot_url,
    payment_status, group_size, is_group_booking,
    base_amount, addons_amount, total_amount
  )
  VALUES (
    p_event_id, p_user_id, p_custom_data, p_payment_screenshot_url,
    v_payment_status, p_group_size, p_is_group_booking,
    v_base_amount, COALESCE(v_addons_amount, 0), v_total_amount
  )
  ON CONFLICT (event_id, user_id) DO NOTHING
  RETURNING id INTO v_registration_id;

  -- If no row was inserted, concurrent insert happened
  ...
  IF v_event.is_paid = false THEN
    ...
    INSERT INTO event_tickets (
      id, event_id, user_id, registration_id, qr_code,
      is_used, is_group_booking, group_size
    )
    VALUES (
      v_ticket_id, p_event_id, p_user_id, v_registration_id, v_qr_code,
      false, p_is_group_booking, p_group_size
    )
  ...
  RETURN jsonb_build_object(
    ...
    'message', CASE WHEN v_event.is_paid 
      THEN 'Registration submitted! Payment pending verification.'
