-- Fix register_for_event_atomic to handle FREE events correctly with idempotent behavior
...
BEGIN
  ...
  -- Insert registration with ON CONFLICT to handle race conditions
...
-- Ensure unique constraint exists for idempotent registrations
DO $$
BEGIN
  IF NOT EXISTS (
