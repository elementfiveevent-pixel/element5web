import { PrismaClient, UserRole, UserStatus, EventCategory } from "@prisma/client";
...
async function main() {
  ...
  // 2. Seed Default Achievements / Badges
  const achievements = [
    {
      title: "Rising Star",
  ...
  const organizer = await prisma.user.upsert({
    ...
    create: {
      ...
      status: UserStatus.ACTIVE,
      roles: {
        create: { role: UserRole.ORG_ADMIN },
      },
    },
  });
  ...
  // 6. Create a Demo Event (StageVerse category)
  ...
  const event = await prisma.event.upsert({
    ...
    create: {
      ...
      category: EventCategory.STAGEVERSE,
