...
exports.PrismaService = exports.PostgresModel = void 0;
...
class PostgresModel {
    ...
    tableName;
    service;
    constructor(pool, tableName, service) {
        this.pool = pool;
        this.tableName = tableName;
        this.service = service;
        const originalQuery = this.pool.query.bind(this.pool);
        this.pool.query = (async (sql, values) => {
    ...
    async loadIncludes(rows, include) {
        ...
        for (const row of rows) {
            for (const [relation, includeVal] of Object.entries(include)) {
                ...
                if (relation === "roles" && this.tableName === "User") {
                    const res = await this.pool.query(`SELECT * FROM "RoleAssignment" WHERE "userId" = $1`, [row.id]);
                    row.roles = res.rows;
                }
                else if (relation === "user" && this.tableName === "RefreshToken") {
                    const userRes = await this.pool.query(`SELECT * FROM "User" WHERE "id" = $1`, [row.userId]);
                    row.user = userRes.rows[0] || null;
                    if (row.user && typeof includeVal === "object" && includeVal.include) {
                        await this.service.user.loadIncludes([row.user], includeVal.include);
                    }
                }
                    ...
                    const locRes = await this.pool.query(`SELECT * FROM "Location" WHERE "eventId" = $1`, [row.id]);
                    row.location = locRes.rows[0] || null;
                }
                else if (relation === "ticketCategories" && this.tableName === "Event") {
                    const catRes = await this.pool.query(`SELECT * FROM "TicketCategory" WHERE "eventId" = $1`, [row.id]);
                    row.ticketCategories = catRes.rows;
                }
                else if (relation === "organizer" && this.tableName === "Event") {
                    const orgRes = await this.pool.query(`SELECT * FROM "User" WHERE "id" = $1`, [row.organizerId]);
                    row.organizer = orgRes.rows[0] || null;
                }
                else if (relation === "registrations" && this.tableName === "Event") {
                    const regsRes = await this.pool.query(`SELECT * FROM "EventRegistration" WHERE "eventId" = $1`, [row.id]);
                    row.registrations = regsRes.rows;
                }
                else if (relation === "tickets" && this.tableName === "Event") {
                    const ticketsRes = await this.pool.query(`SELECT * FROM "EventTicket" WHERE "eventId" = $1`, [row.id]);
                    row.tickets = ticketsRes.rows;
                }
                else if (relation === "checkInAuditLogs" && this.tableName === "Event") {
                    const logsRes = await this.pool.query(`SELECT * FROM "CheckInAuditLog" WHERE "eventId" = $1 ORDER BY "createdAt" ASC`, [row.id]);
                    row.checkInAuditLogs = logsRes.rows;
                }
                else if (relation === "event" && this.tableName === "EventRegistration") {
                    const eventRes = await this.pool.query(`SELECT * FROM "Event" WHERE "id" = $1`, [row.eventId]);
                    ...
                    row.user = userRes.rows[0] || null;
                    if (row.user) {
                        const rolesRes = await this.pool.query(`SELECT * FROM "RoleAssignment" WHERE "userId" = $1`, [row.user.id]);
                        row.user.roles = rolesRes.rows;
                    }
                }
                else if (relation === "tickets" && this.tableName === "EventRegistration") {
                    const ticketsRes = await this.pool.query(`SELECT * FROM "EventTicket" WHERE "registrationId" = $1`, [row.id]);
                    row.tickets = ticketsRes.rows;
                }
                    ...
                    const profileRes = await this.pool.query(`SELECT * FROM "ArtistProfile" WHERE "userId" = $1`, [row.id]);
                    row.artistProfile = profileRes.rows[0] || null;
                }
                else if (relation === "submission" && this.tableName === "Vote") {
                    const subRes = await this.pool.query(`SELECT * FROM "StageVerseSubmission" WHERE "id" = $1`, [row.submissionId]);
                    row.submission = subRes.rows[0] || null;
                }
                else if (relation === "votes" && this.tableName === "StageVerseSubmission") {
                    const votesRes = await this.pool.query(`SELECT * FROM "Vote" WHERE "submissionId" = $1`, [row.id]);
                    row.votes = votesRes.rows;
                }
                else if (relation === "judgeScores" && this.tableName === "StageVerseSubmission") {
                    const scoresRes = await this.pool.query(`SELECT * FROM "JudgeScore" WHERE "submissionId" = $1`, [row.id]);
                    row.judgeScores = scoresRes.rows;
                }
                else if (relation === "event" && this.tableName === "EventTicket") {
                    const eventRes = await this.pool.query(`SELECT * FROM "Event" WHERE "id" = $1`, [row.eventId]);
                    row.event = eventRes.rows[0] || null;
                    if (row.event && typeof includeVal === "object" && includeVal.include) {
                        await this.service.event.loadIncludes([row.event], includeVal.include);
                    }
                }
                else if (relation === "registration" && this.tableName === "EventTicket") {
                    const regRes = await this.pool.query(`SELECT * FROM "EventRegistration" WHERE "id" = $1`, [row.registrationId]);
    ...
    async findMany(args) {
        ...
        if (args?.orderBy) {
            if (Array.isArray(args.orderBy)) {
                const parts = args.orderBy.map((o) => {
    ...
    async create(args) {
        ...
        const placeholders = [];
        let placeholderIndex = 1;
        let nestedCreation = null;
        for (const [key, value] of Object.entries(data)) {
            if (value === undefined)
                continue;
            if (typeof value === "object" && value !== null && !(value instanceof Date) && !Array.isArray(value) && value.create) {
                if (key === "roles") {
                    const roleVal = value.create.role;
                    nestedCreation = async () => {
                        ...
                        const now = new Date();
                        if (roleId) {
                            await this.pool.query(`INSERT INTO "RoleAssignment" ("id", "userId", "role", "createdAt") VALUES ($1, $2, $3, $4)`, [roleId, insertedRow.id, roleVal, now]);
                        }
                        else {
                            await this.pool.query(`INSERT INTO "RoleAssignment" ("userId", "role", "createdAt") VALUES ($1, $2, $3)`, [insertedRow.id, roleVal, now]);
                        }
                    };
                }
                else if (key === "location") {
                    const locData = { ...value.create };
                    nestedCreation = async () => {
                        ...
                        await this.pool.query(`INSERT INTO "Location" ("eventId", ${locCols}) VALUES ($1, ${locPlaces})`, [insertedRow.id, ...locVals]);
                    };
                }
                else if (key === "ticketCategories") {
                    const catsData = value.create;
                    nestedCreation = async () => {
                        ...
                        for (const cat of cats) {
                            ...
                            await this.pool.query(`INSERT INTO "TicketCategory" ("eventId", ${catCols}) VALUES ($1, ${catPlaces})`, [insertedRow.id, ...catVals]);
                        }
                    };
                }
                continue;
        ...
        for (const [key, value] of Object.entries(data)) {
            ...
            if (typeof value === "object" && value !== null && !(value instanceof Date) && !Array.isArray(value)) {
                ...
                if (ops.includes("increment")) {
                    ...
                    setValues.push(value.increment);
                }
                continue;
            }
...
exports.PostgresModel = PostgresModel;
let PrismaService = PrismaService_1 = class PrismaService {
    ...
    async onModuleInit() {
        try {
            ...
            this.logger.warn(`⚠ PostgreSQL unavailable: ${err?.message ?? err}. Running without database.`);
        }
    }
