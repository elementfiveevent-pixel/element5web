...
let AuthService = class AuthService {
    ...
    async googleLogin(idToken) {
        ...
        try {
            ...
            if (parts.length !== 3) {
                throw new common_1.UnauthorizedException("Invalid Google token format");
            }
            const payload = JSON.parse(Buffer.from(parts[1], "base64").toString("utf-8"));
            email = payload.email;
            fullName = payload.name || email.split("@")[0];
            if (!email) {
                throw new common_1.UnauthorizedException("Google token missing email information");
            }
        }
        catch (err) {
            throw new common_1.UnauthorizedException("Invalid Google authentication payload");
        }
